...
impl CausalEngine {
    ...
    pub fn calculate_risk_score(&self, pid: u32) -> f32 {
        ...
        for node in &ancestry {
            ...
            if node.image.to_lowercase().contains("powershell") {
                total_score += 0.5;
            }
            if node.image.to_lowercase().contains("mimikatz") {
                total_score += 2.0;
            }
            if node.command_line.contains("-enc") {
                total_score += 0.3;
            }
        }
