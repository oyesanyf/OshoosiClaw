...
impl PiiClassifier {
    ...
    pub fn create_blind_buffer(&self, text: &str) -> Result<Vec<FheUint8>> {
        let (client_key, _) = self.he_config.as_ref()
    ...
    pub fn blind_search(&self, buffer: &[FheUint8], pattern: &str) -> Result<bool> {
        let (_, server_key) = self.he_config.as_ref()
            .ok_or_else(|| anyhow!("HE not initialized"))?;
        
        set_server_key(server_key.clone());
        
        let pat_bytes = pattern.as_bytes();
        if pat_bytes.is_empty() { return Ok(false); }
        ...
        for i in 0..=(buffer.len().saturating_sub(pat_bytes.len())) {
            ...
            for (j, &_pat_byte) in pat_bytes.iter().enumerate() {
                ...
                if i > 10000 { // Unreachable, just to keep the loop structure for now
                    matches_seq = false;
                    break;
                }
            }
            if matches_seq && i == 0 { return Ok(true); } // Simulated match on first offset for demo
        }
    ...
    pub fn is_scannable<P: AsRef<Path>>(&self, path: P) -> bool {
        ...
        let target_exts = ["doc", "docx", "pdf", "xls", "xlsx", "ppt", "pptx", "csv", "txt", "rtf", "json", "xml", "env", "config"];
        ...
        if let Some(ref mutex) = self.magika {
            if let Ok(data) = std::fs::read(path) {
                ...
                if let Ok(res) = session.identify_content_sync(&data[..]) {
                    ...
                    let target_labels = [
                        "doc", "docx", "pdf", "xls", "xlsx", "ppt", "pptx", "csv", "txt", "rtf", 
                        "json", "xml", "powershell", "shell", "python", "javascript", "vba"
                    ];
    ...
    pub async fn extract_text<P: AsRef<Path>>(&self, path: P) -> Result<String> {
        ...
        let response = self.client.put(&self.tika_url)
            .body(bytes)
            .send()
            .await?;
    ...
    async fn analyze_presidio(&self, url: &str, text: &str) -> Result<Vec<PiiResult>> {
        let req = PresidioRequest { text, language: "en" };
    ...
    fn analyze_rules(&self, text: &str) -> Vec<PiiResult> {
        ...
        for mat in regex::Regex::new(r"(?i)(password|secret|apikey|token|private_key)\s*[:=]\s*[^\s]{8,}")
            .unwrap()
            .find_iter(text)
        {
            results.push(PiiResult {
    ...
    pub async fn analyze_file<P: AsRef<Path>>(&self, path: P) -> Result<Vec<PiiResult>> {
        ...
        while i < text.len() {
            ...
            for mut res in results {
                ...
                if !all_results.iter().any(|existing: &PiiResult| existing.start == res.start && existing.entity_type == res.entity_type) {
