...
fn windows_check(status: &mut PrivilegeStatus) {
    ...
    status
        .details
        .push(format!("Event Log Readers member: {}", in_group));
    ...
    status.details.push(format!(
        "Readable channels: {}/{}",
        readable.len(),
        WINDOWS_LOG_CHANNELS.len()
    ));
    if !unreadable.is_empty() {
        status
            .details
            .push(format!("Unreadable: {}", unreadable.join(", ")));
    }
...
fn windows_grant(status: &mut PrivilegeStatus) {
    ...
    info!(
        "Granting read-only access to all event logs for {}...",
        user
    );
    ...
    let sysmon_check = Command::new("sc")
        .args(["query", "Sysmon64"])
        .output()
    ...
    match sysmon_check {
        ...
        _ => {
            status
                .actions_taken
                .push("Sysmon not found. Please run 'osoosi install-telemetry' next.".into());
        }
    }
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        Ok(_) => status
            .actions_taken
            .push("Enforced Process Creation Auditing (CLI) via native API.".into()),
        Err(e) => status.details.push(format!(
            "Failed to set CLI auditing via Registry API: {}",
            e
        )),
    }
    ...
    if ollama_check.is_err() || !ollama_check.unwrap().status.success() {
        status
            .actions_taken
            .push("Ollama NOT FOUND. AI Behavioral Analysis will be disabled.".into());
        status
            .actions_taken
            .push("HINT: Run 'winget install Ollama' or visit ollama.com".into());
    } else {
    ...
    if status.can_apply_patches {
        status
            .details
            .push("Repair Engine: Can apply patches (running elevated).".into());
    } else {
...
fn grant_channel_read(status: &mut PrivilegeStatus, channel: &str, sid: Option<&str>) {
    ...
    let sid = match sid {
        ...
        None => {
            status
                .details
                .push(format!("{}: skipped SDDL (no SID)", channel));
            return;
        }
    };
    ...
    if current_sddl.is_empty() {
        status
            .details
            .push(format!("{}: could not parse SDDL", channel));
        return;
    }
    ...
    if current_sddl.contains(sid) {
        status
            .details
            .push(format!("{}: SID already in SDDL", channel));
        return;
    }
        ...
        .output()
    {
        Ok(o) if o.status.success() => {
            status
                .actions_taken
                .push(format!("{}: read ACE granted", channel));
        ...
        Ok(o) => {
            ...
            status
                .details
                .push(format!("{}: SDDL update skipped ({})", channel, msg.trim()));
        }
        Err(e) => {
            status
                .details
                .push(format!("{}: SDDL error ({})", channel, e));
        }
    }
}
...
fn linux_check(status: &mut PrivilegeStatus) {
    ...
    if !status.can_read_events {
        status
            .details
            .push("Fix: run `sudo osoosi grant-access`".into());
    }
    if !status.can_apply_patches {
...
fn linux_grant(status: &mut PrivilegeStatus) {
    ...
    if !status.is_elevated {
        status
            .errors
            .push("Must run as root (sudo) to grant access.".into());
        status.can_read_events = false;
        return;
    }
    ...
    for &group in LINUX_LOG_GROUPS {
        match Command::new("usermod").args(["-aG", group, &user]).output() {
            Ok(o) if o.status.success() => {
                status
                    .actions_taken
                    .push(format!("Added {} to {} group", user, group));
                info!("Added {} to {}", user, group);
            }
            Ok(o) => {
                ...
                if e.to_lowercase().contains("does not exist") {
                    status
                        .details
                        .push(format!("{} group does not exist (OK)", group));
                } else {
    ...
    for &path in LINUX_LOG_FILES {
            ...
            .output()
        {
            Ok(o) if o.status.success() => {
                status
                    .actions_taken
                    .push(format!("Set read ACL on {}", path));
            }
            Ok(o) => {
                let e = String::from_utf8_lossy(&o.stderr);
                if e.contains("not found") || e.contains("command not found") {
                    status
                        .details
                        .push("setfacl not installed. Install: apt install acl".into());
                    break;
                }
            ...
            Err(_) => {
                status
                    .details
                    .push("setfacl not available. Install: apt install acl".into());
                break;
            }
        }
    }
    ...
    let _ = Command::new("sudo")
        .args(["systemctl", "enable", "--now", "auditd"])
        .status();
    let _ = Command::new("sudo")
        .args([
            "auditctl",
    ...
    status
        .actions_taken
        .push("Enabled auditd and added execve monitoring rule.".into());
        ...
        .unwrap_or(false)
    {
        ...
        status
            .details
            .push("Sysmon for Linux detected and enabled.".into());
    } else {
        status
            .actions_taken
            .push("Sysmon for Linux not found. Please run 'osoosi install-telemetry' next.".into());
    }
...
fn macos_check(status: &mut PrivilegeStatus) {
    ...
    status
        .details
        .push(format!("Unified log readable: {}", can_log));
    status
        .details
        .push(format!("system.log readable: {}", can_system_log));
...
fn macos_grant(status: &mut PrivilegeStatus) {
    ...
    if status.is_elevated {
            ...
            .output()
        {
            Ok(o) if o.status.success() => {
                status
                    .actions_taken
                    .push(format!("Confirmed {} in admin group", user));
            }
            _ => {
                status
                    .details
                    .push(format!("{} likely already in admin group", user));
            }
        }
        ...
        for path in &mac_logs {
            if std::path::Path::new(path).exists() {
                let _ = Command::new("chmod").args(["+r", path]).output();
    ...
    if !can_log {
        status
            .details
            .push("Full Disk Access still required for unified log (manual step).".into());
    }
    if !status.can_apply_patches {
        status.details.push(
            "Repair Engine: Run with sudo for auto-patch (softwareupdate requires root).".into(),
        );
    }
}
        ...
        if let Ok(exe_path) = std::env::current_exe() {
            ...
            let _ = Command::new("netsh")
                ...
                .args([
                    ...
                    "add",
                    "rule",
                    "name=OpenOsoosi-Dashboard-8080",
                    "dir=in",
                    "action=allow",
                    "protocol=TCP",
                    "localport=8080",
            ...
            let _ = Command::new("netsh")
                ...
                .args([
                    ...
                    "add",
                    "rule",
                    "name=OpenOsoosi-Dashboard-3030",
                    "dir=in",
                    "action=allow",
                    "protocol=TCP",
                    "localport=3030",
