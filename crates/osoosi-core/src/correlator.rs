...
impl EventCorrelator {
    ...
    pub async fn correlate_sysmon(&self, event: &SysmonEvent) -> Option<ThreatSignature> {
        ...
        let image = event.data.get("Image").and_then(|i| i.as_str()).unwrap_or("unknown");

        let mut ctx = self.processes.entry(pid).or_insert_with(|| ProcessContext::new(pid, image.to_string()));
        ...
        match event_type.as_str() {
            "1" => { // Process Creation
                ...
                let command_line = event.data.get("CommandLine").and_then(|v| v.as_str()).unwrap_or("");
                
                // Suspicious: PowerShell with encoded commands or discovery tools
                if command_line.contains("-enc") || command_line.contains("whoami") || command_line.contains("net user") {
                    alert_score = 0.3;
                    reason = format!("Suspicious command line execution: {}", command_line);
                }
                ctx.add_event("ProcessCreate", &format!("Parent: {} | Cmd: {}", parent_image, command_line), alert_score);
            },
            "3" => { // Network Connection
                let dest_ip = event.data.get("DestinationIp").and_then(|v| v.as_str()).unwrap_or("");
                let dest_port = event.data.get("DestinationPort").and_then(|v| v.as_str()).unwrap_or("");
                
                // Check if process has C2 capabilities from CAPA/Static analysis
                let has_c2_cap = ctx.static_findings.iter().any(|f| f.contains("communication") || f.contains("c2") || f.contains("network"));
                
                if has_c2_cap {
                    alert_score = 0.5; // High boost if static capabilities match dynamic behavior
                    reason = format!("Process with forensic C2 markers initiated network connection to {}:{}", dest_ip, dest_port);
                } else if !ctx.is_signed {
                    alert_score = 0.15;
                    reason = format!("Unsigned process initiated network connection to {}:{}", dest_ip, dest_port);
                }
                
                ctx.add_event("Network", &format!("To {}:{}", dest_ip, dest_port), alert_score);
            },
            "11" => { // File Create
                let target_path = event.data.get("TargetFilename").and_then(|v| v.as_str()).unwrap_or("");
                if target_path.contains("\\Windows\\System32") || target_path.contains("AppData\\Roaming") {
                    alert_score = 0.2;
                    reason = format!("Process created file in sensitive directory: {}", target_path);
                }
                ctx.add_event("FileCreate", target_path, alert_score);
            },
            "13" => { // Registry Value Set
                let target_key = event.data.get("TargetObject").and_then(|v| v.as_str()).unwrap_or("");
                if target_key.contains("\\CurrentVersion\\Run") || target_key.contains("\\Services\\") {
                    alert_score = 0.3;
        ...
        if alert_score > 0.3 || ctx.total_score > 0.7 {
            ...
            sig.recommended_action = if ctx.total_score > 0.85 { ResponseAction::Isolate } else { ResponseAction::Alert };
    ...
    pub fn add_static_finding(&self, pid: u32, image: &str, finding: &str, confidence: f32) {
        ...
        info!("Correlator: Added static finding for PID {}: {} (New Score: {:.2})", pid, finding, ctx.total_score);
    }
    ...
    pub fn get_process_summary(&self, pid: u32) -> Option<String> {
        ...
        let timeline_str = ctx.timeline.iter()
            .map(|e| format!("[{}] {}: {}", e.timestamp.format("%H:%M:%S"), e.event_type, e.details))
            .collect::<Vec<_>>()
            .join("\n");
    ...
    pub async fn investigate_with_llm(&self, pid: u32, llm: &osoosi_behavioral::SmolLMAnalyzer) -> anyhow::Result<Option<ThreatSignature>> {
