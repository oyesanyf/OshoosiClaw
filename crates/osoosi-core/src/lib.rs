...
impl EdrOrchestrator {
                ...
                match poll_result.unwrap_or(Err(anyhow::anyhow!("Poll task panicked"))) {
                    Ok(events) => {
                        for event in events {
                            ...
                            // more anomaly detections.
                            let is_self = event.data.get("ProcessName")
                                .or(event.data.get("Image"))
                                .or(event.data.get("NewProcessName"))
                                .and_then(|v| v.as_str())
                                .map(|s| {
                                    let lower = s.to_ascii_lowercase();
                                    lower.contains("osoosi") || lower.contains("oshoosi")
                                })
                                .unwrap_or(false);
                            if is_self { continue; }
                ...
                if let Ok(entropy) = static_analyzer.calculate_entropy(&path) {
                    ...
                    if entropy < 6.5 && is_browser {
                        if signature.is_some() {
                            signature = None; // Suppression logic
                        }
                    } else if entropy > 7.5 {
                        if let Some(ref mut sig) = signature {
                            sig.confidence = (sig.confidence + 0.15).min(1.0);
                            sig.add_reason(format!(
                                "Static Analysis: High entropy detected ({:.2}), suggesting packed/obfuscated binary.",
                                entropy
                            ));
                        }
                    }
    ...
    async fn handle_threat(
        ...
        // If high confidence, trigger Burst mode to prioritize telemetry
        if signature.confidence > 0.6 {
            let _ = self.adaptive.trigger_burst_mode(60).await;
        }
