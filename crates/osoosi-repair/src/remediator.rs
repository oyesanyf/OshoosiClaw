...
impl StandaloneRemediator {
    ...
    pub async fn remediate_file(&self, target_path: &str, download_url: &str) -> Result<PathBuf> {
        info!("Remediating standalone file: {} via {}", target_path, download_url);
        
        let target = Path::new(target_path);
        if !target.exists() {
            return Err(anyhow!("Target file for remediation does not exist: {}", target_path));
        }

        // 1. Download to memory first to ensure it's valid
        let response = self.client.get(download_url).send().await?;
        if !response.status().is_success() {
            return Err(anyhow!("Failed to download remediation file from {}: {}", download_url, response.status()));
        }
        ...
        if let Err(e) = fs::rename(&temp_file, target) {
            error!("Failed to swap remediated file: {}. Attempting rollback from backup.", e);
    ...
    pub fn rollback(&self, target_path: &str, backup_path: &Path) -> Result<()> {
        warn!("Rolling back remediation for {} from {:?}", target_path, backup_path);
        if !backup_path.exists() {
            return Err(anyhow!("Backup file missing for rollback: {:?}", backup_path));
        }
