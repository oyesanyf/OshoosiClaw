...
#[cfg(target_os = "windows")]
use winapi::um::winnt::{PAGE_EXECUTE_READWRITE, PROCESS_ALL_ACCESS};
...
impl LiveProcessPatcher {
    ...
    pub fn patch_process_memory(pid: u32, offset: usize, new_data: &[u8]) -> anyhow::Result<()> {
        info!(
            "JIT: Patching process {} at offset 0x{:x} ({} bytes)",
            pid,
            offset,
            new_data.len()
        );
            ...
            unsafe {
                ...
                if handle.is_null() {
                    return Err(anyhow::anyhow!(
                        "Failed to open process {} for patching",
                        pid
                    ));
                }
                ...
                ) == 0
                {
                    CloseHandle(handle);
                    return Err(anyhow::anyhow!(
                        "Failed to change memory protection for process {}",
                        pid
                    ));
                }
                ...
                if WriteProcessMemory(
                    handle,
                    offset as *mut winapi::ctypes::c_void,
                    new_data.as_ptr() as *const winapi::ctypes::c_void,
                    new_data.len(),
                    &mut bytes_written,
                ) == 0
                {
                    ...
                    CloseHandle(handle);
                    return Err(anyhow::anyhow!(
                        "Failed to write memory for process {}",
                        pid
                    ));
                }
