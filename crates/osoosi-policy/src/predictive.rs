...
pub fn predict_next_step(
    ...
    if proc.contains("vssadmin") && cmd.contains("delete shadows") {
        return Some(
            "Volume shadow deletion often precedes credential dumping or ransomware encryption"
                .to_string(),
        );
    }
    ...
    if proc.contains("whoami") || proc.contains("net.exe") || cmd.contains("dir /s") {
        return Some(
            "Discovery phase; may progress to credential access or lateral movement".to_string(),
        );
    }
    ...
    if proc.contains("powershell")
        && (cmd.contains("invoke") || cmd.contains("download") || cmd.contains("bypass"))
    {
