...
impl ReasoningEngine {
    pub fn new() -> Self {
        ...
        let backend_type =
            std::env::var("OSOOSI_REASONING_BACKEND").unwrap_or_else(|_| "api".to_string());
        let backend = if backend_type == "python" {
            ReasoningBackend::PythonBridge {
                script_path: std::env::var("OSOOSI_REASONING_SCRIPT")
                    .unwrap_or_else(|_| "scripts/reason.py".to_string()),
            }
    ...
    pub async fn reason(
        ...
        match &self.backend {
            ...
            ReasoningBackend::RemoteApi { url, api_key } => {
                self.call_remote_api(url, api_key, &prompt).await
            }
            ReasoningBackend::PythonBridge { script_path } => {
                self.call_python_bridge(script_path, &prompt).await
            }
        }
    }
        ...
        events
            ...
            .map(|e| {
                format!(
                    ...
                    e.data
                        .get("Message")
                        .unwrap_or(&serde_json::json!("no_msg"))
                )
            })
            .collect::<Vec<_>>()
            .join("\n")
    ...
    async fn call_smollm(&self, prompt: &str) -> anyhow::Result<ReasoningResult> {
        ...
        let smollm = smollm_guard.as_ref().ok_or_else(|| {
            anyhow::anyhow!("SmolLM analyzer not yet initialized (still loading in background)")
        })?;
        ...
        Ok(ReasoningResult {
            ...
            explanation: format!(
                "Native SmolLM2-135M analysis identified high suspicion score: {:.2}",
                score
            ),
            confidence: score,
    ...
    fn parse_ai_output(&self, text: &str) -> ReasoningResult {
        ...
        let is_malicious =
            text.to_lowercase().contains("malicious") || text.to_lowercase().contains("threat");

        ReasoningResult {
            verdict: if is_malicious {
                "Malicious".to_string()
            } else {
                "Benign".to_string()
            },
            ...
            recommended_yara_l: if is_malicious {
                Some("rule Suggestion { ... }".to_string())
            } else {
                None
            },
        }
