use osoosi_types::{TaintLabel, TaintSink, TaintedValue};
...
fn test_taint_flow_logical() {
    ...
    let tainted = TaintedValue::new("C:\\Users\\Public\\malware.exe", labels, "NetworkDownload");
    ...
    assert!(
        result.is_err(),
        "Downloaded file should be blocked from process injection"
    );
}
