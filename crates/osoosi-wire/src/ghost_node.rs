...
impl GhostNodeManager {
    ...
    pub async fn spawn_ghost_service(&self, port: u16, service_name: &str) {
        ...
        info!("Coy Deception: Ghost service '{}' active on {}", service_name, addr);
            ...
            loop {
                match listener.accept().await {
                    Ok((mut socket, addr)) => {
                        ...
                        let noisy_addr = if jitter > 0.5 { addr.to_string() } else { "REDACTED".to_string() };
                        
                        warn!("DECEPTION ALERT: Interaction from {} on '{}' (Node: {})", noisy_addr, service_name, node_id);
                        
                        // Notify mesh (Tarpit)
                        let tx_guard = mesh_tx.lock().await;
                        if let Some(ref tx) = *tx_guard {
                            let _ = tx.try_send(crate::MeshCommand::BroadcastTarpit(crate::TarpitSignal {
                        ...
                        let banner: &[u8] = match service_name.as_str() {
                            ...
                            "SMB" => b"\x00\x00\x00\x85\xffSMB\x72\x00\x00\x00\x00\x18\x53\xc8\x00\x00",
