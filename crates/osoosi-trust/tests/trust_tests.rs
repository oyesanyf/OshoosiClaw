...
fn test_did_generation() {
    ...
    assert!(did.id.starts_with("did:osoosi:"), "DID should use the osoosi prefix");
    assert_eq!(did.public_key.len(), 64, "Public key hex should be 64 characters (32 bytes)");
}
...
fn test_ca_init_structure() {
    ...
    if let Err(e) = tm.init_ca(temp_dir.to_str().unwrap()) {
        if e.to_string().contains("failed to fill whole buffer") || e.to_string().contains("not found") {
